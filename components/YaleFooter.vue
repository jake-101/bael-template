<template>
<footer class="fill-gray-lighter xs-text-6 md-text-5">
    <div class="r no-gap">
      
       <div class="c-12 xs-text-left xs-p2 xs-border">
              <p class="item bold">&nbsp;</p>
       </div>
           <div class="c-25 xs-text-left xs-p2 xs-border">
              <div class="item"><div class="footer__heading xs-mb2">About</div>
<p>A fashionable brutalist blog template using Netlify CMS and Nuxt.js.</p>
</div>
       </div>
             <div class="c-25 xs-text-left xs-p2 xs-border">
            <div class="item"><div class="footer__heading xs-mb2">Newsletter</div>
       <form netlify>
  <label class="form-label form-label--small">Sign up on our newsletter</label>
  <input type="email" class="text-input text-input--small xs-mb1 xs-mr2" placeholder="you@email.com">
  <button class="button button--transparent button--small">Submit</button>
</form></div>
       </div>
             <div class="c-25 xs-text-left xs-p2 xs-border">
               <div class="item"><div class="footer__heading xs-mb2">Connect</div>
               <ul class="list-unstyled">
  <li><a href="https://instagram.com/jasperketone">Instagram</a></li>
  <li><a href="https://twitter.com/jasperketone">Twitter</a></li>
  <li><a href="https://github.com/jake101">Github</a></li>
</ul></div>
       </div>
             <div class="c-25 xs-text-left xs-p2 xs-border">
               <div class="item"><div class="footer__heading xs-mb2">Contact</div>
               <ul class="list-unstyled">
  <li>323.963.3240</li>
</ul></div>
       </div>
      <div class="c-12 xs-text-left xs-p2 xs-border">
              <p class="item xs-text-6">An open source design by <a href="https://jake101.com">jake101</a>, inspired by Balenciaga.com</p>
       </div>
       </div>
</footer>
</template>